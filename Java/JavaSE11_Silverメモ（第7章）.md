...
‐ コンストラクタ
  - コンストラクタとは：**インスタンスを準備するためのメソッド。全てのインスタンスが持たなければいけない**
  - 定義しなかった場合は、**デフォルトコンストラクタ**が自動的に追加される
  - スーパークラスを継承していた場合、スーパークラスのコンストラクタが先に実行される
```
public B {
　super();
  // do anything
...
- 第12章総仕上げ問題 問42復習
  - **スーパークラスとサブクラスに同じ名前のフィールドがあった場合、そのインスタンスへの参照が何型で扱われているかで、どちらのフィールドが使われるかが決まる**
  - この問題の場合、Bクラスのインスタンスへの参照をA型の変数で扱っている
  - Aに定義したフィールドxが使われる

【検証用コード】  
https://github.com/kuro-channel/TIL/blob/master/%E6%A4%9C%E8%A8%BC%E3%82%B3%E3%83%BC%E3%83%89/src/jv/silver/twelve/ex42/Main.java#L8-L10
